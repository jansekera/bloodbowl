#!/bin/bash
# ============================================================================
# NOC 27.→28.08.2026 — M1/N10 (BLITZ JE POHYB S BLOKEM UVNITŘ)
#
...
# ============================================================================
set -u
ROOT=/home/jan/claude/bloodbowl   # ⚠️ 27.08.: větev m1-n10 musí být v té chvíli MERGNUTÁ do main
cd "$ROOT" || exit 1

LIGHT=${LIGHT:-0}
if [ "$LIGHT" = "1" ]; then
    OUT_DIR=blitzcont_lighttest_20260827
    ...
    LOG=$ROOT/night_20260827.log
fi

say() { echo "[$(date '+%m-%d %H:%M')] $*" | tee -a "$LOG"; }

say "=== NOC 27.08. START (LIGHT=$LIGHT, OUT=$OUT_DIR) ==="
...
MATCHUPS="1:dw-we:1" \
PREREG="$ROOT/evidence/night_prereg_20260827.preds" \
OUT="$OUT_DIR" \
