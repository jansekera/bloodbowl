...
# cap-arm mini-A/B (test 3, evidence/fable_teacher_signal_report_20260810.md
# par 1.3): rows come from the FROZEN A/B corpus in evidence/, results go to
# evidence/diag_teacher_signal_20260810/. Cap arm = old 23 features + 8
# capability/risk features nf[0,1,2,11,12,13,14,15] (= [23-25]+[34-38] of the
# full set), NO target-identity features (nf[3..10] excluded).
ROWS_DIR = "/home/jan/claude/bloodbowl/evidence/diag_feature_ab_20260810"
OUT_DIR = "/home/jan/claude/bloodbowl/evidence/diag_teacher_signal_20260810"
CAP_IDX = [0, 1, 2, 11, 12, 13, 14, 15]
...
OUT_SUFFIX = os.environ.get("AB_SUFFIX", "_cap")
...
def build_inputs(dec, arm):
    ...
    if arm == "old":
        ...
        X = np.zeros((n, N_SF + N_F + len(CAP_IDX)))
        X[:, :N_SF] = dec["sf"]
        X[:, N_SF:N_SF + N_F] = dec["f"]
        X[:, N_SF + N_F:] = dec["nf"][:, CAP_IDX]
    return X


def train(decs, arm, seed):
    rng = np.random.default_rng(seed)
    n_in = N_SF + N_F + (len(CAP_IDX) if arm == "cap" else 0)
    net = MLP(n_in, rng)
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "results_ab" + OUT_SUFFIX + ".json"), "w") as f:
    ...
    text = "\n".join(lines)
    print(text)
    with open(os.path.join(OUT_DIR, "results_ab" + OUT_SUFFIX + ".out"), "w") as f:
        f.write(text + "\n")
