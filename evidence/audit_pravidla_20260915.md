...
| ⭐ **Dodge — přehoz 1× za kolo CHYBÍ** *(nález agenta u Pro/Sure Feet 18.09.; řádek pravidel ověřen ručně)* | 8089-8090, 960-962: „may only re-roll one failed Dodge roll per turn“ | `Action/MoveHandler.php` ~:249 přehoz úhybu bez limitu | OK *(`dodgeRerollUsedThisTurn`, `helpers.cpp:307`)* | – | jistá |
| **Pro — chytání po odskoku a throw-inu** *(nález agenta 18.09.)* | 1263 + 8381: Pro na jakýkoli hod kromě brnění/zranění | `BallResolver::resolveCatchFromBounce` Pro nenabízí; přehoz Catch tam nevydá událost | ⏰ nekontrolováno | – | pravděpodobná |
...
| Pro | 8381, 926: 1× za kolo, kostka nejvýš 1× přehozena, 1-3 = výsledek platí | `BallResolver.php:62,164`, `PassResolver.php:142` nenastaví used; Pro po skill přehozu; po Pro týmový přehoz (`MoveHandler.php:308,442`); blok jen nejhorší kostka (`BlockHandler.php:320-340,690`) | OK | – | jistá |
| Sure Feet | 8541: 1× za kolo | `Action/MoveHandler.php:396` bez limitu | OK | – | jistá |
...
| ⏰ **TTM: hozený NOSIČ do davu — míč zmizí** *(sestra throw-inu, 18.09., neopraveno — skupina 4 TTM)* | 8614-8616 „beaten up by the crowd in the same manner as a player who has been pushed“ ⇒ throw-in r. 659-663 | `Action/ThrowTeamMateHandler.php:183` `BallState::offPitch()` | ⏰ nekontrolováno | – | jistá |
