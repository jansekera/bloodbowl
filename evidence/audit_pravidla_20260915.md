...
| ✅ **Break Tackle — OPRAVENO 21.09.** | 7988-7991: „**may** use his Strength instead of his Agility … may only be used **once per turn**" | `TacklezoneCalculator::breakTackleAvailable()` — síla jen když je VYŠŠÍ než obratnost a jen 1× za kolo; `MoveHandler` počítá cíl úhybu znovu ze současného stavu (cesta se počítá předem, kdy je skill ještě volný) | limit chybí | `BreakTackleRulesTest.php` (4 testy) | jistá |
...
| ✅ **C++: blok po neúspěšném Pro — OPRAVENO 21.09.** | 8386-8387: „the original result stands and **may not be re-rolled with a skill or team re-roll**; however you can re-roll the Pro roll with a Team re-roll" | `engine/src/block_handler.cpp` — po neúspěšném Pro jde týmový přehoz na HOD PRO (i s Lonerem) a kostky bloku se už nepřehazují (`proAttempted`) | ✅ opraveno | `tests/test_block_handler.cpp` (2 testy) | jistá |
...
| ✅ **TTM: −1 k hodu, jen quick/short, fumble na původní pole — OPRAVENO 21.09.** | 8607-8613: „must **subtract 1** from the D6 roll"; „**Long Pass or Long Bomb range passes are not possible**"; „A **fumbled team-mate will land in the square he originally occupied**" | `ThrowTeamMateHandler` počítá modifikátor jako u přihrávky (`range − TZ − 1`), fumble i z modifikovaného hodu (r. 1742-1745); `RulesEngine::validateThrowTeamMate` long/long bomb odmítne ⇒ **není to ani v nabídce** | – | `ThrowTeamMateTest.php` (+5 testů) | jistá |
...
| ✅⭐ **Faul** — **OPRAVENO 16.09.** *(asistence, paušální +1 pryč, turnover)* **a 21.09.** *(dublet i na ZRANĚNÍ)*; ✅ **i volba u Dirty Player (21.09.)** | 1845-1849 asistence; **1877-1878 dublet zbroj a/nebo zranění**; 384 vyloučení = turnover | `Action/FoulHandler.php` čte dublet z obou hodů; `InjuryResolver::resolveInjury` vrací i jednotlivé kostky (`dice`) — dřív jen součet `roll2D6()` | OK | `FoulTest.php` (+3 testy 21.09.) | jistá |
