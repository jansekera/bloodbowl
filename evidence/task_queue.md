...
// Strategy 3: Endzone guard
    ...
    guardY = (endzoneGuardCount == 0) ? 5 : 9;   // PEVNÉ Y</pre>⇒ **hrozbu DETEKUJE, a místo aby ji ZNAČKOVAL, pošle dvě těla na natvrdo daná pole y=5 a y=9.** V situaci `g0004` stojí hrozba na **y=13** ⇒ gardisté by minuli o 4-8 polí. **Vidí ho a jde jinam.**<br>⇒ ⭐⭐ **Dvě opravy, každá jiné třídy:** *(1)* **pořadí** — Strategy 2 *(nosič)* jde dnes **před** Strategy 3 *(hrozba)*; uživatelovo pravidlo to **obrací**. *(2)* **odpověď** — značkovat toho konkrétního hráče, ne obsadit pevná pole *(to je konkrétní škoda k `T1.12`: nejde jen o tvar, ale o to, že odpověď s detekcí nemá nic společného)*.<br>⭐ **Měřený rámec už je:** `P59` spočítalo, že **v 29 % kol vede k TD cesta, kterou umí JEN přihrávka**. Tohle je ta cesta, a tohle je místo v kódu, kde se dnes míjí. | **OTEVŘENO — ⏰⏰ vstup pro `B-ROUND` i `CELOTAH`** |
