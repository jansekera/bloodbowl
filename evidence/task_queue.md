...
| P1 | **Přepsat K33 a K34 na spojité** — jako prahy nepředpovídají nic (0,6σ / 0,8σ), jako počty patří k nejsilnějším. Platí i pro E1: *„ani jeden otevřený roh"* je správný **cíl**, ale jako **kontrola** se má měřit `REACH0` jako počet (1 → 8,3 %, 4+ → 33 % ztráty). | **POVÝŠENO 18.08.** — uživatel: *„spravit kontroly vždy povýšit před měření“*. σ-tabulka 18.08.: REACH0 jako **počet −16,7σ** vs ano/ne 9,3σ; bloky jako **počet +10,4σ** vs ano/ne **−2,5σ**, a to ano/ne v půlkách korpusu **nereplikuje** ⇒ **K33 dnes neměří nic a přitom vypadá jako splněná kontrola**, tedy dává radu s obráceným znaménkem. Povinnost zůstává ano/ne, metr si nechá číslo. |
...
| **0a.** | **P1** — K33/K34 na počty | povýšeno pravidlem výše; dnes se zahazuje 2. nejsilnější prediktor |
| **0b.** | **T2.14** — předpovědi se nekonfrontují s výsledkem | poslední vada čtecí strany noci |
| **0c.** | **K9b** — odpor ven z brány | předpoklad T0.1 (přepis K9 na fáze) |
