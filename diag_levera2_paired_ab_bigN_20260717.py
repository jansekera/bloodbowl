...
big-N follow-up to the N=150 run (diag_levera2_paired_ab_20260717.py,
base_seed=20260726), which came back INCONCLUSIVE (draws 47.3% off vs 43.3%
on, delta -4.0pp, CI [-13.4,+5.4]pp; home_win +0.7pp, CI [-7.1,+8.4]pp --
both include zero). Per the project noise-floor convention (<10pp at N=150 =
inconclusive), this re-runs the exact same A/B at N=400 with a fresh base
seed to tell "truly no effect" from "real but small effect".

The single-state deepdive (engine_build_levera2_test/arm_off.out /
arm_on.out, 2026-07-17) found the reworked lever (a) --
estimateMarkedCarrierDodgeFail() using the real calculateDodgeTarget() over
the carrier's legal exit squares, gating an ADVANCE prior-floor bump
0.12->0.20 in MacroMCTSSearch::expand() when the marked carrier's dodge-fail
estimate is <=30% -- produces the largest single-state effect measured today
on the flagship stalled-carrier scenario: ADVANCE chosen 3.8%->27.0% (7x).
...
WORKERS deliberately reduced 8->4: several other background jobs are running
concurrently today and the box is near its 12-core capacity.

Usage: python3 diag_levera2_paired_ab_bigN_20260717.py <arm>|all|smoke|compare
"""
import os
import sys
...
TV, MCTS = 1000, 100
BASE_SEED = 20260728   # house sequence: next free (20260726=lever-a2 N150,
                       # 20260727=reposition N400)
WORKERS = 4
N = 400
...
def path_for(arm: str, smoke: bool = False) -> str:
    ...
    return f"arm_levera2_ab_bigN_{tag}{arm}_20260717.json"
...
def run(arm: str, n: int | None = None, smoke: bool = False) -> None:
    ...
    print(f"=== lever-a2 A/B bigN{' SMOKE' if smoke else ''}: arm={arm} N={n} "
