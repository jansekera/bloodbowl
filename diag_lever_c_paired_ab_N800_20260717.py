#!/usr/bin/env python3
"""Paired-seed A/B: lever (c) stallPacing-discount, N=800 follow-up.

History: N=150 (base_seed=20260723) draw delta -2.0pp, CI [-13.3,+9.3]pp --
inconclusive. N=400 (diag_lever_c_paired_ab_bigN_20260717.py,
base_seed=20260725) draw delta -6.2pp, CI [-13.3,+0.8]pp, p=0.0951 --
still formally inconclusive but the upper bound barely clears zero and the
direction is consistent across both sample sizes (and matches the shipped
CAGE/ADVANCE fixes of the same bug class). This re-runs the exact same A/B
at N=800 with a fresh base seed to see whether it clears the significance
bar or falls back toward null.
...
Usage: python3 diag_lever_c_paired_ab_N800_20260717.py <arm>|all|compare
"""
import os
import sys
...
TV, MCTS = 1000, 100
BASE_SEED = 20260729   # house sequence: next free (20260727=reposition N400, 20260728=lever-a2 N400)
WORKERS = 8
N = 800
...
def path_for(arm: str) -> str:
    return f"arm_leverc_ab_N800_{arm}_20260717.json"
...
def run(arm: str) -> None:
    ...
    print(f"=== lever-c A/B N800: arm={arm} N={a['n']} lever_c={a['lever']} "
