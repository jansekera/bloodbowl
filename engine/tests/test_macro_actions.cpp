...
// W-GFI (04.09.2026, NASAZENO 09.09.2026, nepodmineny). Pred timhle
// rozhodnutim expandReposition NIKDY nedovolil volnemu hraci GFI (hardcoded
// +0 headroom) -- chuze se zastavila o pole driv i kdyz bylo riziko levne.
// Ted, kdyz uz neni koho ztratit, cena P_fail * zbyvajici je vzdy < 1, takze
// se GFI povoli; fixni 6 ho nechá uspet.
TEST(MacroExpansion, RepositionGfiClosesTheGapWhenTeammatesAreFree) {
    GameState state = makeMinimalState();
    Player& p1 = state.getPlayer(1);
    p1.position = {10, 7};
    ...
    FixedDiceRoller dice(std::vector<int>(10, 6));
    greedyExpandMacro(state, macro, dice);
    EXPECT_EQ(state.getPlayer(1).position, target)
        << "no teammate left to lose, GFI succeeds -- must reach it";
}
...
// must stay conservative -- price, not a blank check.
TEST(MacroExpansion, RepositionGfiStaysConservativeWhenExpensive) {
    GameState state = makeMinimalState();
    Player& p1 = state.getPlayer(1);
    p1.position = {10, 7};
    p1.movementRemaining = 3;
