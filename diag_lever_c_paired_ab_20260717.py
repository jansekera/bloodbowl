#!/usr/bin/env python3
"""Paired-seed A/B: lever (c) stallPacing-discount, game-level follow-up to
the single-state deepdive (engine_build_lever_test/arm_*_v2.out, 2026-07-17).

That deepdive found lever (c) alone produces a large, clean effect on the
exact stalled-carrier decision (ADVANCE chosen 3.8%->16.2% marked,
46.5%->68.2% unmarked), fully isolated from lever (a) which measured zero
effect there (bug-fixed re-run, see project memory
project_bloodbowl_relaunch_20260717.md). This script checks whether that
single-state signal survives to actual game-level draw-rate/TD-rate metrics.

Uses a SEPARATE python extension built against the scratch lever engine
(engine_build_lever_test/bb_engine.cpython-312-*.so, dynamically linked to
engine_build_lever_test/libbb_engine.so which has the getenv-gated lever
code) -- NOT the production engine/build module. BB_LEVER_C is set in this
process's environ before the worker Pool is created so forked/spawned
workers inherit it; BB_LEVER_A is left unset throughout (isolating lever c
only, since lever a measured zero effect on the flagship scenario and
wasn't worth the extra arms here given the time budget).

Usage: python3 diag_lever_c_paired_ab_20260717.py <arm>|all|compare
"""
import os
import sys
...
TV, MCTS = 1000, 100
BASE_SEED = 20260723   # house sequence: next free (20260721=vf retest, 20260722=policyblend)
WORKERS = 8
N = 150
...
def path_for(arm: str) -> str:
    return f"arm_leverc_ab_{arm}_20260717.json"
...
def run(arm: str) -> None:
    ...
    print(f"=== lever-c A/B: arm={arm} N={a['n']} lever_c={a['lever']} "
